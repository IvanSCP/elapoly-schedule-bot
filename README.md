...
**Управление загрузкой файлов:** Администраторы могут загружать новый файл расписания непосредственно в бот, а также обновлять файл расписания в один клик, используя парсинг URL-адреса сайта ЕПК.
...
```/fc```: _File change._ Позволяет администраторам загружать новый файл расписания.

```/up```: _URL parse._ Позволяет администраторам обновлять расписание с помощью парсинга URL-адреса сайта ЕПК.


## Проверка прав доступа 
Для предотвращения высокой наргузки на сервер, парсить расписание могут только пользователи, находящиеся в определённой группе/канале (по логике, студенты). 

Имеется возможность выдать доступ пользователям, отсутствующим в группе/канале, путём занесения их ID в файл ```permissions.txt```. 
...
В ```date_column``` и ```time_column``` по имолчанию указана колонка B, так как в ней находятся ячейки с датам и временем.
...
### buttons_text
Тексты кнопок клавиатуры, отображающейся в чате с пользователем.
...
**Администраторам** позволяет использовать команды ```/fc``` и ```/up```.

Не даёт пользоваться парсингом **заблокированным** пользователям.

## schedule_parser.py
Парсит расписание в файле ```schedule.xsls```.
